...
def create_cv_page_summary_request(property_id, start_date, end_date, cv_events, path_filter=None, limit=20):
    ...
    # pathフィルターの構築（create_page_summary_request と同じロジック）
    ...
    if path_filter:
        ...
        if len(path_list) == 1:
            ...
            path_filter_expr = FilterExpression(
                filter=Filter(
                    field_name='pagePath',
                    string_filter=Filter.StringFilter(value=path_list[0], match_type=Filter.StringFilter.MatchType.EXACT)
                )
            )
        elif len(path_list) > 1:
            # 複数パス（OR条件）
            ...
            path_filter_expr = FilterExpression(or_group=FilterExpressionList(expressions=filters))

    # pathフィルターとイベントフィルターをAND条件で組み合わせる
...
def process_site_summary(response):
    ...
    if not response or not response.rows:
        return {
            "total_users": 0, "new_users": 0, "sessions": 0, "page_views": 0,
            "engagement_rate": 0.0, "avg_session_duration": 0.0, "bounce_rate": 0.0,
    ...
    return {
        "total_users": total_users, "new_users": new_users, "sessions": sessions,
        "page_views": page_views, "engagement_rate": engagement_rate,
        "avg_session_duration": avg_session_duration, "bounce_rate": bounce_rate,
...
def process_cv_dimension_data(response, dimension_key, event_names):
    ...
    for row in response.rows:
        ...
        if dim_value not in result_map:
            result_map[dim_value] = {dimension_key: dim_value}
            result_map[dim_value].update({event: 0 for event in event_names})
...
def process_cv_hour_data(response, event_names):
    ...
    for hour_str, events in sorted(hour_data.items(), key=lambda item: int(item[0])):
        hour_item = {"hour": int(hour_str)}
        hour_item.update(events)
...
def execute_summary_requests_parallel(client, requests, max_workers=3):
    ...
    try:
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            future_to_index = {
                executor.submit(client.run_report, request=req): i 
            ...
            for future in as_completed(future_to_index):
                ...
                try:
                    ...
                    # logger.info(f"集計リクエスト{index+1}完了: {len(response.rows) if response else 0}行")
