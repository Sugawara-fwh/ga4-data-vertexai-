...
import tiktoken  # トークン数計算用ライブラリを追加
import re
...
def fetch_ga4_data(property_id: str, start_date: str, end_date: str, cv_events: str = "click_lpo", row_limit: int = 11000, path_filter: str = "") -> Dict[str, Any]:
    ...
    try:
      ...
      # --- 修正版 end ---
        ...
        lp_paths = None
        if path_filter:
            # path_filterからLPパスを抽出（例: "/entry04/,/entry05/,/entry06/"）
            path_list = [p.strip() for p in path_filter.split(',') if p.strip()]
            # LPパスとして認識されるパスを抽出
            lp_paths = [path for path in path_list if '/entry' in path or '/lp' in path.lower()]
            if lp_paths:
